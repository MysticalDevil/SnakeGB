...
auto createFruitEntry(const int type, const QString &name, const QString &description, const bool discovered)
    -> QVariantMap {
    QVariantMap map;
...
auto buildFruitLibraryModel(const QList<int> &discoveredFruitTypes) -> QVariantList {
    ...
    for (const auto &fruit : fruits) {
        result.append(createFruitEntry(fruit.type, fruit.name, fruit.description,
                                       discoveredFruitTypes.contains(fruit.type)));
    }
    return result;
}
