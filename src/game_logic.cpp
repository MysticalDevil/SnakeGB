...
void GameLogic::selectChoice(int index) {
    ...
    if (m_activeBuff == Mini) {
        ...
        for(size_t i = 0; i < half; ++i) nb.push_back(body[i]);
...
void GameLogic::loadLastSession() {
    ...
    auto d = m_profileManager->loadSession();
    m_score = d[u"score"_s].toInt();
    m_food = d[u"food"_s].toPoint();
    m_direction = d[u"dir"_s].toPoint();
    m_obstacles.clear();
    
    for (const auto &v : d[u"obstacles"_s].toList()) {
        m_obstacles.append(v.toPoint());
    }
    
    std::deque<QPoint> b;
    for (const auto &v : d[u"body"_s].toList()) {
        b.emplace_back(v.toPoint());
    }
...
void GameLogic::setInternalState(int s) {
    ...
    if (m_state != next) {
        m_state = next;
        if (m_soundManager) {
            m_soundManager->setPaused(m_state == Paused || m_state == ChoiceSelection || m_state == Library || m_state == MedalRoom);
        }
        emit stateChanged();
    }
}
...
void GameLogic::handleFoodConsumption(const QPoint &head) {
    ...
    int points = 1;
    if (m_activeBuff == Double) points = 2;
    else if (m_activeBuff == Rich) points = 3;
    
    m_score += points;
    if (m_profileManager) m_profileManager->logFoodEaten();
...
void GameLogic::triggerHaptic(int magnitude) { 
    ...
    if (context.isValid()) {
        ...
        if (vibrator.isValid()) {
            vibrator.callMethod<void>("vibrate", "(J)V", static_cast<jlong>(magnitude * 15));
        }
    }
#endif
}
...
void GameLogic::updatePersistence() {
    updateHighScore();
    if (m_profileManager) m_profileManager->incrementCrashes();
    clearSavedState();
}

void GameLogic::startEngineTimer(int intervalMs) {
    if (intervalMs > 0) m_timer->setInterval(intervalMs);
    m_timer->start();
}
...
void GameLogic::togglePause() {
    if (m_state == Playing) requestStateChange(Paused);
    else if (m_state == Paused) requestStateChange(Playing);
}
...
void GameLogic::nextShellColor() {
    if (m_profileManager) {
        // 经典 7 色方案
        m_profileManager->setShellIndex((m_profileManager->shellIndex() + 1) % 7);
...
void GameLogic::nextLevel() {
    ...
    loadLevelData(m_levelIndex);
    emit levelChanged();
    if (m_profileManager) m_profileManager->setLevelIndex(m_levelIndex);
}

void GameLogic::toggleMusic() {
    if (m_soundManager) {
        bool e = !m_soundManager->musicEnabled();
        m_soundManager->setMusicEnabled(e);
        if (e && m_state != Splash) m_soundManager->startMusic();
...
void GameLogic::checkAchievements() {
    ...
    auto unlock = [this](const QString &t) {
        if (m_profileManager->unlockMedal(t)) {
            emit achievementEarned(t);
            emit achievementsChanged();
        }
    };
    if (m_score >= 50) unlock(u"Gold Medal (50 Pts)"_s);
    if (m_timer->isActive() && m_timer->interval() <= 60) unlock(u"Speed Demon"_s);
}
...
bool GameLogic::isOutOfBounds(const QPoint &p) noexcept {
...
void GameLogic::update() {
    if (m_fsmState) {
        if (m_activeBuff != None && m_buffTicksRemaining > 0) {
            if (--m_buffTicksRemaining <= 0) deactivateBuff();
        }
        m_fsmState->update();
...
void GameLogic::spawnFood() {
    ...
    for (int x = 0; x < BOARD_WIDTH; ++x) {
        for (int y = 0; y < BOARD_HEIGHT; ++y) {
            QPoint p(x, y);
            if (!isOccupied(p) && p != m_powerUpPos) freeSpots << p;
        }
    }
...
void GameLogic::spawnPowerUp() {
    ...
    for (int x = 0; x < BOARD_WIDTH; ++x) {
        for (int y = 0; y < BOARD_HEIGHT; ++y) {
            QPoint p(x, y);
            if (!isOccupied(p) && p != m_food) freeSpots << p;
        }
    }
    if (!freeSpots.isEmpty()) {
...
bool GameLogic::isOccupied(const QPoint &p) const {
    for (const auto &bp : m_snakeModel.body()) if (bp == p) return true;
    for (const auto &op : m_obstacles) if (op == p) return true;
    return false;
}
...
QVariantList GameLogic::achievements() const {
    QVariantList list;
    if (m_profileManager) {
        for (const auto &m : m_profileManager->unlockedMedals()) list.append(m);
    }
    return list;
}
...
QColor GameLogic::shellColor() const {
    // 工业级 7 色经典配色方案
    static const QList<QColor> c = {
        u"#c0c0c0"_s, // Gray (Classic)
        u"#f0f0f0"_s, // Off-white (Pocket)
        u"#9370db"_s, // Atomic Purple
        u"#ff0000"_s, // Berry (Red)
        u"#008080"_s, // Teal
        u"#ffd700"_s, // Dandelion (Yellow)
        u"#2f4f4f"_s  // Dark Gray
    };
    int idx = m_profileManager ? m_profileManager->shellIndex() % 7 : 0;
    return c[idx];
}
QVariantList GameLogic::ghost() const {
    if (m_state == Replaying) return {};
    QVariantList list;
...
QVariantList GameLogic::medalLibrary() const {
    QVariantList list;
...
void GameLogic::setVolume(float v) {
    if (m_profileManager) m_profileManager->setVolume(v);
    if (m_soundManager) m_soundManager->setVolume(v);
    emit volumeChanged();
}
