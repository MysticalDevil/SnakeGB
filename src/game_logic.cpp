...
void GameLogic::nextPalette() {
    if (m_profileManager) {
        int idx = (m_profileManager->paletteIndex() + 1) % 5;
        m_profileManager->setPaletteIndex(idx);
        emit paletteChanged();
...
void GameLogic::nextShellColor() {
    if (m_profileManager) {
        int idx = (m_profileManager->shellIndex() + 1) % 7;
        m_profileManager->setShellIndex(idx);
...
void GameLogic::generateChoices() {
    ...
    QList<ChoiceInfo> allChoices = {
        {Ghost, u"Ghost"_s, u"Pass through self"_s},
        {Slow, u"Slow"_s, u"Decrease speed"_s},
        {Magnet, u"Magnet"_s, u"Attract food"_s},
        {Shield, u"Shield"_s, u"One extra life"_s},
        {Portal, u"Portal"_s, u"Screen wrap"_s},
        {Double, u"Double"_s, u"Double points"_s},
        {Rich, u"Diamond"_s, u"Triple points"_s},
        {Laser, u"Laser"_s, u"Break obstacle"_s},
        {Mini, u"Mini"_s, u"Shrink body"_s}
    };
    std::shuffle(allChoices.begin(), allChoices.end(), std::default_random_engine(m_rng.generate()));
    for (int i = 0; i < 3; ++i) {
        QVariantMap m; m.insert(u"type"_s, allChoices[i].type); m.insert(u"name"_s, allChoices[i].name); m.insert(u"desc"_s, allChoices[i].desc);
        m_choices.append(m);
    }
    emit choicesChanged();
}
...
void GameLogic::selectChoice(int index) {
    ...
    if (m_activeBuff == Mini) {
        auto body = m_snakeModel.body(); if (body.size() > 3) {
            std::deque<QPoint> nb; size_t half = std::max<size_t>(3, body.size()/2);
            for(size_t i=0; i<half; ++i) nb.push_back(body[i]); m_snakeModel.reset(nb);
        } m_activeBuff = None;
    }
...
void GameLogic::loadLastSession() {
    ...
    m_score = data[u"score"_s].toInt(); m_food = data[u"food"_s].toPoint(); m_direction = data[u"dir"_s].toPoint(); m_obstacles.clear();
    for (const auto &v : data[u"obstacles"_s].toList()) m_obstacles.append(v.toPoint());
    std::deque<QPoint> body; for (const auto &v : data[u"body"_s].toList()) body.emplace_back(v.toPoint()); m_snakeModel.reset(body);
    m_inputQueue.clear(); for (const auto &p : body) m_currentRecording.append(p);
    m_timer->setInterval(std::max(60, 200 - (m_score / 5) * 8)); m_timer->start();
    emit scoreChanged(); emit foodChanged(); emit obstaclesChanged(); emit ghostChanged(); requestStateChange(Paused);
}

void GameLogic::setInternalState(int s) {
    State next = static_cast<State>(s); if (m_state != next) {
        m_state = next; if (m_soundManager) m_soundManager->setPaused(m_state == Paused || m_state == ChoiceSelection || m_state == Library || m_state == MedalRoom);
        emit stateChanged();
    }
}
...
bool GameLogic::checkCollision(const QPoint &head) {
    ...
    else if (isOutOfBounds(p)) { if (m_shieldActive) { m_shieldActive = false; triggerHaptic(5); emit buffChanged(); return false; } return true; }
    for (int i = 0; i < m_obstacles.size(); ++i) {
        if (m_obstacles[i] == p) {
            ...
            if (m_shieldActive) { m_shieldActive = false; triggerHaptic(5); emit buffChanged(); return false; } return true;
        }
    }
...
void GameLogic::update() {
    if (m_fsmState) {
        if (m_activeBuff != None && m_buffTicksRemaining > 0) { if (--m_buffTicksRemaining <= 0) deactivateBuff(); }
        m_fsmState->update(); if (!m_currentScript.isEmpty()) runLevelScript(); m_gameTickCounter++;
    }
    float t = static_cast<float>(QDateTime::currentMSecsSinceEpoch()) / 1000.0f;
    m_reflectionOffset = QPointF(std::sin(t * 0.8f) * 0.01f, std::cos(t * 0.7f) * 0.01f); emit reflectionOffsetChanged();
}

void GameLogic::handleFoodConsumption(const QPoint &head) {
    QPoint p = head; if (m_activeBuff == Portal) { p.setX((p.x() + BOARD_WIDTH) % BOARD_WIDTH); p.setY((p.y() + BOARD_HEIGHT) % BOARD_HEIGHT); }
    if (p != m_food) return;
    int points = (m_activeBuff == Double) ? 2 : (m_activeBuff == Rich ? 3 : 1);
...
void GameLogic::playEventSound(int type, float pan) {
    if (type == 0) emit foodEaten(pan); else if (type == 1) emit playerCrashed(); else if (type == 2) emit uiInteractTriggered(); else if (type == 3 && m_soundManager) m_soundManager->playBeep(150, 100);
}
...
void GameLogic::lazyInit() { if (m_profileManager) { m_levelIndex = m_profileManager->levelIndex(); if (m_soundManager) m_soundManager->setVolume(m_profileManager->volume()); } QFile f(getGhostFilePath()); if (f.open(QIODevice::ReadOnly)) { QDataStream in(&f); quint32 magic; in >> magic; if (magic == GHOST_FILE_MAGIC) in >> m_bestRecording >> m_bestRandomSeed >> m_bestInputHistory >> m_bestLevelIndex >> m_bestChoiceHistory; else if (magic >= 0x534E4B02) { in >> m_bestRecording >> m_bestRandomSeed >> m_bestInputHistory >> m_bestLevelIndex; m_bestChoiceHistory.clear(); } } loadLevelData(m_levelIndex); spawnFood(); emit paletteChanged(); emit shellColorChanged(); }
void GameLogic::lazyInitState() { if (!m_fsmState) changeState(std::make_unique<SplashState>(*this)); }
...
QVariantList GameLogic::palette() const { static const QList<QVariantList> p = {{u"#cadc9f"_s, u"#8bac0f"_s, u"#306230"_s, u"#0f380f"_s}, {u"#e0e8d0"_s, u"#a0a890"_s, u"#4d533c"_s, u"#1f1f1f"_s}, {u"#ffd700"_s, u"#e0a000"_s, u"#a05000"_s, u"#201000"_s}, {u"#00ffff"_s, u"#008080"_s, u"#004040"_s, u"#002020"_s}, {u"#ff0000"_s, u"#a00000"_s, u"#500000"_s, u"#200000"_s}}; return p[m_profileManager ? m_profileManager->paletteIndex() % 5 : 0]; }
QString GameLogic::paletteName() const { static const QStringList n = {u"Original DMG"_s, u"Pocket B&W"_s, u"Golden Lux"_s, u"Ice Blue"_s, u"Virtual Red"_s}; return n[m_profileManager ? m_profileManager->paletteIndex() % 5 : 0]; }
QColor GameLogic::shellColor() const { static const QList<QColor> c = {u"#c0c0c0"_s, u"#f0f0f0"_s, u"#9370db"_s, u"#ff0000"_s, u"#008080"_s, u"#ffd700"_s, u"#2f4f4f"_s}; return c[m_profileManager ? m_profileManager->shellIndex() % 7 : 0]; }
...
void GameLogic::checkAchievements() { if (!m_profileManager) return; auto unlock = [this](const QString &t) { if (m_profileManager->unlockMedal(t)) { emit achievementEarned(t); emit achievementsChanged(); } }; if (m_score >= 50) unlock(u"Gold Medal (50 Pts)"_s); if (m_timer->isActive() && m_timer->interval() <= 60) unlock(u"Speed Demon"_s); }
void GameLogic::updateHighScore() { if (m_profileManager && m_score > m_profileManager->highScore()) { m_profileManager->updateHighScore(m_score); m_bestInputHistory = m_currentInputHistory; m_bestRecording = m_currentRecording; m_bestChoiceHistory = m_currentChoiceHistory; m_bestRandomSeed = m_randomSeed; m_bestLevelIndex = m_levelIndex; QFile file(getGhostFilePath()); if (file.open(QIODevice::WriteOnly)) { QDataStream out(&file); out << GHOST_FILE_MAGIC << m_bestRecording << m_bestRandomSeed << m_bestInputHistory << m_bestLevelIndex << m_bestChoiceHistory; } emit highScoreChanged(); } }
void GameLogic::loadLevelData(int i) { QFile f(u"qrc:/src/levels/levels.json"_s); if (!f.open(QIODevice::ReadOnly)) return; auto levels = QJsonDocument::fromJson(f.readAll()).object().value(u"levels"_s).toArray(); auto lvl = levels[i % levels.size()].toObject(); m_currentLevelName = lvl.value(u"name"_s).toString(); m_obstacles.clear(); m_currentScript = lvl.value(u"script"_s).toString(); if (!m_currentScript.isEmpty()) { m_jsEngine.evaluate(m_currentScript); runLevelScript(); } else { for (const auto &w : lvl.value(u"walls"_s).toArray()) m_obstacles.append(QPoint(w.toObject().value(u"x"_s).toInt(), w.toObject().value(u"y"_s).toInt())); } emit obstaclesChanged(); }
QVariantList GameLogic::fruitLibrary() const { QVariantList list; QList<int> discovered = m_profileManager ? m_profileManager->discoveredFruits() : QList<int>(); auto add = [&](int t, QString n, QString d) { bool isDiscovered = discovered.contains(t); QVariantMap m; m.insert(u"type"_s, t); m.insert(u"name"_s, isDiscovered ? n : u"??????"_s); m.insert(u"desc"_s, isDiscovered ? d : u"Eat this fruit in-game to unlock its data."_s); m.insert(u"discovered"_s, isDiscovered); list << m; }; add(Ghost, u"Ghost"_s, u"Pass through yourself."_s); add(Slow, u"Slow"_s, u"Slows the game down."_s); add(Magnet, u"Magnet"_s, u"Standard nutritious food."_s); add(Shield, u"Shield"_s, u"Survive one collision."_s); add(Portal, u"Portal"_s, u"Allows screen wrapping."_s); add(Double, u"Golden"_s, u"2x points per food."_s); add(Rich, u"Diamond"_s, u"3x points per food."_s); add(Laser, u"Laser"_s, u"Breaks one obstacle."_s); add(Mini, u"Mini"_s, u"Shrinks body by 50%."_s); return list; }
