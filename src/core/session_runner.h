...
public:
    ...
    void startReplay(QList<QPoint> obstacles, uint randomSeed, QList<ReplayFrame> inputHistory,
    ...
    [[nodiscard]] auto core() -> SessionCore & { return m_core; }
    [[nodiscard]] auto core() const -> const SessionCore & { return m_core; }
    [[nodiscard]] auto mode() const -> SessionMode { return m_mode; }
    [[nodiscard]] auto randomSeed() const -> uint { return m_randomSeed; }
    [[nodiscard]] auto choices() const -> const QList<ChoiceSpec> & { return m_choices; }
    [[nodiscard]] auto recording() const -> const QList<QPoint> & { return m_recording; }
    [[nodiscard]] auto inputHistory() const -> const QList<ReplayFrame> & { return m_inputHistory; }
    [[nodiscard]] auto choiceHistory() const -> const QList<ChoiceRecord> & { return m_choiceHistory; }
