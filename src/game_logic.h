...
public:
    ...
    int rowCount(const QModelIndex &parent = QModelIndex()) const override { return static_cast<int>(m_body.size()); }
    ...
    void moveHead(const QPoint& newHead, bool grew) {
        beginInsertRows(QModelIndex(), 0, 0); m_body.push_front(newHead); endInsertRows();
        if (!grew) {
