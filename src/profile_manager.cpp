...
ProfileManager::ProfileManager(QObject* parent)
  ...
  for (const auto& v : fruitList)
    m_discoveredFruits << v.toInt();
}

void ProfileManager::setPaletteIndex(int i) {
  m_paletteIndex = i;
  m_settings.setValue(u"paletteIndex"_s, i);
}
void ProfileManager::setShellIndex(int i) {
  m_shellIndex = i;
  m_settings.setValue(u"shellIndex"_s, i);
}
void ProfileManager::setLevelIndex(int i) {
  m_levelIndex = i;
  m_settings.setValue(u"levelIndex"_s, i);
}
void ProfileManager::setVolume(float v) {
  m_volume = v;
  m_settings.setValue(u"volume"_s, v);
}
void ProfileManager::updateHighScore(int s) {
  m_highScore = s;
  m_settings.setValue(u"highScore"_s, s);
}
...
auto ProfileManager::hasSession() const -> bool {
  return m_settings.value(u"session/active"_s, false).toBool();
}
