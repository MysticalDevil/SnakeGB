...
void ProfileManager::setPaletteIndex(int index) {
    m_paletteIndex = index;
    m_settings.setValue(u"paletteIndex"_s, m_paletteIndex);
}

void ProfileManager::setShellIndex(int index) {
    m_shellIndex = index;
...
void ProfileManager::saveSession(int score, const std::deque<QPoint> &body, const QList<QPoint> &obstacles, QPoint food, QPoint dir) {
    ...
    m_settings.setValue(u"saved_score"_s, score);
    m_settings.setValue(u"saved_body"_s, bodyVar);
    m_settings.setValue(u"saved_obstacles"_s, obsVar);
    m_settings.setValue(u"saved_food"_s, food);
    m_settings.setValue(u"saved_dir"_s, dir);
    m_settings.sync();
}
...
bool ProfileManager::hasSession() const {
    return m_settings.contains(u"saved_body"_s);
}

QVariantMap ProfileManager::loadSession() {
    QVariantMap data;
    data.insert(u"score"_s, m_settings.value(u"saved_score"_s));
    data.insert(u"body"_s, m_settings.value(u"saved_body"_s));
    data.insert(u"obstacles"_s, m_settings.value(u"saved_obstacles"_s));
    data.insert(u"food"_s, m_settings.value(u"saved_food"_s));
    data.insert(u"dir"_s, m_settings.value(u"saved_dir"_s));
    return data;
}
