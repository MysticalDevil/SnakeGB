...
auto SoundManager::generateSquareWave(const int frequencyHz, const int durationMs,
                                      QByteArray &buffer, int amplitude, double duty, float pan)
    -> void
{
...
void SoundManager::generateNoise(const int durationMs, QByteArray &buffer)
{
    ...
    for (int i = 0; i < frames; ++i) {
        ...
        const int randVal =
            QRandomGenerator::global()->bounded(Noise_Amplitude * 2) - Noise_Amplitude;
